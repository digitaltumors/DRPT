...
class TransferCellResponse(object):
    ...
    def train_pycox_CT(self, dataloader, batch_size):
        ...
        age =  batch['age'].detach().cpu().numpy()
        sex = batch['sex'].detach().cpu().numpy()
        time = batch['time'].detach().cpu().numpy()
        event = batch['event'].detach().cpu().numpy()

        # Create DF for training hazard transform model
        df_train = pd.DataFrame({'AUC': auc, 'Age': age, 'Sex': sex, 'Time': time, 'Event': event}) 
        df_test = df_train.sample(frac=0.2)
        df_train = df_train.drop(df_test.index)

        # Standardize the features for train and test
        standards = [(['AUC'], StandardScaler()), (['Age'], StandardScaler())]
        binarized = [('Sex', None)]
        x_mapper = DataFrameMapper(standards + binarized)
        x_train = x_mapper.fit_transform(df_train).astype('float32')
        x_test = x_mapper.transform(df_test).astype('float32')

        # Standardize the time labels
        get_target = lambda df: (df['Time'].values, df['Event'].values)
        y_train = self.labtrans.fit_transform(*get_target(df_train))
        y_test = self.labtrans.transform(*get_target(df_test))
        durations_test, events_test = get_target(df_test)
        val = tt.tuplefy(x_test, y_test)

        # Instantiate a Hazard Tranform Cox-Time Model
        func = MLPVanillaCoxTime(self.num_features, self.hidden_layers, dropout=self.transform_dropout)
        model = CoxTime(func, self.optimizer, labtrans=self.labtrans, device=self.device)
        ...
        antolini = ev.concordance_td(method='antolini')
        time_grid = np.linspace(durations_test.min(), durations_test.max(), 100)
        brier_score = ev.integrated_brier_score(time_grid)
        print("Antolini Concordence Index:", antolini)
        print("Integreated Brier Score:", brier_score)
    ...
    def train_pycox_PCH(self, dataloader, batch_size):
        ...
        # Instantiate a Hazard Tranform Piecewise-Hazard Model
        ...
        model = PCHazard(func, self.optimizer, duration_index=self.labtrans.cuts)
        ...
        print("Antolini Concordence Index:", antolini)
        print("Integreated Brier Score:", brier_score)
        print("Negative Binomial Likelihood:", nbll)
    ...
    def train_pycox_LH(self, dataloader, batch_size):
        ...
        out_features = self.labtrans.out_features
        func = tt.practical.MLPVanilla(self.num_features , self.hidden_layers, out_features, dropout=self.transform_dropout)
        model = LogisticHazard(func, self.optimizer, duration_index=self.labtrans.cuts, device=self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patient_repsonse', dest='patient_response', type=str, default="/cellar/users/zwallace/G2PT/data/test_PFS_cisplatin.txt")
    parser.add_argument('--patient_indicators', dest='patient_indicators', type=str, default="/cellar/users/zwallace/G2PT/data/test_cisplatin.txt")
    parser.add_argument('--patient_mutations', dest='patient_mutations', type=str, default="/cellar/users/zwallace/G2PT/data/cell2mutation_cisplatin.txt")
    parser.add_argument('--patient_amplifics', dest='patient_amplifics', type=str, default="/cellar/users/zwallace/G2PT/data/cell2cnamplification_cisplatin.txt")
    parser.add_argument('--patient_deletions', dest='patient_deletions', type=str, default="/cellar/users/zwallace/G2PT/data/cell2cndeletion_cisplatin.txt")
    parser.add_argument('--patient2ind', dest='patient2ind', type=str, default="/cellar/users/zwallace/G2PT/data/cell2ind_cisplatin.txt")
