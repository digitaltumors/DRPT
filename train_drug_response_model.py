...
def main():
    ...
    parser.add_argument('--val_step', help='validate/save every N epochs', type=int, default=20)
    ...
    # ---- NEW: selection metric + "full training" flag ----
    parser.add_argument('--best_model_by', choices=['ema', 'pearson'], default='pearson',
                        help='Which metric to select best checkpoint for test/eval.')
    parser.add_argument('--full_train', action='store_true', default=False,
                        help='If set, train for full --epochs with no validation, early stopping, or test. '
                             'Saves the final model to --out.')

    # ---- NEW: early-stopping toggle (off by default) ----
    parser.add_argument('--early_stop', action='store_true', default=False,
                        help='Enable EMA-based early stopping. If not set, train to --epochs and '
                             'select best checkpoint by --best_model_by.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        print(f"GPU {gpu} rank is {args.rank}")
        dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                world_size=args.world_size, rank=args.rank)
        print(f"GPU {gpu} process initialized")
        torch.cuda.empty_cache()
    ...
    if args.drug_embedding:
        ...
        for _, param in compound_bert.named_parameters():
            param.requires_grad = False
        compound_model = ChemBERTaCompoundModel(compound_bert, args.dropout)
    else:
        print(f"ECFP with radius {args.radius}, and {args.n_bits} bits used for compound encoding")
    ...
    if args.model is not None:
        print(f"Loading Model at {args.model}")
    ...
    if args.gene_embedding:
        ...
        gene_embeddings = np.stack(
            [gene_embedding_dict[key] for key, value in sorted(tree_parser.gene2ind.items(), key=lambda a: a[1])] + [np.zeros(args.hidden_dims)]
        ).astype(np.float32)
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            drug_response_model = torch.nn.parallel.DistributedDataParallel(
                drug_response_model, device_ids=[args.gpu], find_unused_parameters=True
            )
        else:
            print("Distributed training set up")
            drug_response_model.to(device)
            drug_response_model = torch.nn.parallel.DistributedDataParallel(
                drug_response_model, find_unused_parameters=True
            )
    elif args.gpu is not None:
        torch.cuda.set_device(args.gpu)
        drug_response_model = drug_response_model.to(device)
        print(f"Model is loaded at GPU({args.gpu})")
    else:
        drug_response_model = torch.nn.DataParallel(drug_response_model).to(device)

    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % torch.cuda.device_count() == 0):
    ...
    # Validation loader (unless full_train)
    if args.val is not None and not args.full_train:
    ...
    trainer = DrugResponseTrainer(
        drug_response_model, drug_response_dataloader_drug, drug_response_dataloader_cellline,
        device, args, validation_dataloader=val_drug_response_dataloader, fix_embedding=fix_embedding
    )

    trainer.train(args.epochs, args.out)
    print("Done training")

    # ---- Full-train mode: stop here and save final model ----
    if args.full_train:
        final_model = drug_response_model.module if isinstance(drug_response_model, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)) else drug_response_model
        print(f"Saving final (full-train) model to {args.out}")
        torch.save({"arguments": args, "state_dict": final_model.state_dict()}, args.out)
        return

    # ---- Otherwise evaluate the selected "best" checkpoint on TEST ----
    if args.test is not None:
        ...
        best_model = trainer.get_best_model(which=args.best_model_by)
        trainer.evaluate(best_model, test_drug_response_dataloader, 1, name="Test")
        print(f"Saving best({args.best_model_by}) model to {args.out}")
        torch.save({"arguments": args, "state_dict": best_model.state_dict()}, args.out)
    else:
        # No test path provided—save the best checkpoint directly.
        best_model = trainer.get_best_model(which=args.best_model_by)
        print(f"Saving best({args.best_model_by}) model to {args.out}")
        torch.save({"arguments": args, "state_dict": best_model.state_dict()}, args.out)
