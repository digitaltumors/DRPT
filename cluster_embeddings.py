...
class ClusterEmbeddings(object):
    ...
    def analyze(self):
        ...
        for system in systems:
            ...
            cluster_response_df.append(self.analyze_cluster_response(system, cluster_info))
        ...
        cluster_response_df = pd.concat(cluster_response_df, ignore_index = True)

        # Do Benjamini–Hochberg FDR correction for cluster response, doing per drug across all systems
        response_dict = defaultdict(lambda: defaultdict(dict))
        for drug, drug_df in cluster_response_df.groupby('Drug'):
            res_pvals = drug_df['Res-P'].values
            sen_pvals = drug_df['Sen-P'].values
            acc_pvals = drug_df['Acc-P'].values

            res_fdr = multipletests(res_pvals, method='fdr_bh')[1]
            sen_fdr = multipletests(sen_pvals, method='fdr_bh')[1]
            acc_fdr = multipletests(acc_pvals, method='fdr_bh')[1]

            # assign corrected values back into the nested dict
            # zip over rows in drug_df and the corresponding corrected arrays
            for (_, row), r_corr, s_corr, a_corr in zip(drug_df.iterrows(), res_fdr, sen_fdr, acc_fdr):
                sys_name = row['System']
                clust = row['Cluster']
                response_dict[sys_name][clust][drug] = {
                    'Res FDR': float(r_corr),
                    'Sen FDR': float(s_corr),
                    'Acc FDR': float(a_corr)
                }
        
        response_dict = {sys: dict(clusts) for sys, clusts in response_dict.items()}

        # Update the final cluster_results dictionary
        for system in cluster_results.keys():
            cluster_info = cluster_results[system]["Clusters"]
            cluster_response = response_dict[system]
    ...
    def analyze_cluster_genes(self, system, cluster_info):
        ...
        # Initialize empty contingency tables for each alteration type
        alteration_types = ['any', 'mut', 'del', 'amp']
        contingency = {alt: {gene: np.zeros((len(clusters), 2), dtype=int) for gene in genes}
        ...
        for c_idx, cluster in enumerate(clusters):
            ...
            for cell in cells:
                i = self.cell2ind[cell]
                muts = self.mutations[i].split(",")
                dels = self.deletions[i].split(",")
                amps = self.amplifics[i].split(",")

                for gene in genes:
                    g = self.gene2ind[gene]
                    is_mut = muts[g] == "1"
                    is_del = dels[g] == "1"
                    is_amp = amps[g] == "1"
                    is_any = is_mut or is_del or is_amp

                    # any alteration
                    if is_any:
                        contingency['any'][gene][c_idx, 0] += 1
                    else:
                        contingency['any'][gene][c_idx, 1] += 1

                    # mutation only
                    if is_mut:
                        contingency['mut'][gene][c_idx, 0] += 1
                    else:
                        contingency['mut'][gene][c_idx, 1] += 1

                    # deletion only
                    if is_del:
                        contingency['del'][gene][c_idx, 0] += 1
                    else:
                        contingency['del'][gene][c_idx, 1] += 1

                    # amplification only
                    if is_amp:
                        contingency['amp'][gene][c_idx, 0] += 1
                    else:
                        contingency['amp'][gene][c_idx, 1] += 1

        # Run Fisher's exact test for every (cluster, gene, alteration)
        records = []
        for alt in alteration_types:
            for gene, table in contingency[alt].items():
                for c_idx, cluster in enumerate(clusters):
                    # in-cluster vs out-of-cluster counts
                    in_counts  = table[c_idx]
                    out_counts = table.sum(axis=0) - in_counts
                    twobytwo   = np.vstack([in_counts, out_counts])
                
                    _, pval = fisher_exact(twobytwo, alternative="greater")
                    records.append({
                        "Cluster":    cluster,
                        "Gene":       gene,
                        "Alteration": alt,
        ...
        reject, pvals_corrected, _, _ = multipletests(
            df_sig["P-Value"], alpha=0.05, method="fdr_bh"
        )
        df_sig["FDR"] = pvals_corrected
        df_sig = df_sig.drop(columns=["P-Value"])

        # Build nested profiles: { cluster: { gene: { alt_type: FDR, … }, … }, … }
        cluster_profiles = {}
        for cluster, grp in df_sig.groupby("Cluster"):
            prof = {}
            for _, row in grp.iterrows():
                gene = row["Gene"]
                alt  = row["Alteration"]
                fdr  = row["FDR"]
                prof.setdefault(gene, {})[alt] = fdr
            cluster_profiles[cluster] = prof

        # Attach back into cluster_info
        for cluster in clusters:
            cluster_info[cluster]["Profiles"] = cluster_profiles.get(cluster, {})

        return cluster_info

    def analyze_cluster_response(self, system, cluster_info):
        ...
        for drug in self.drugs:
            ...
            for cluster in clusters:
                ...
                data = pd.DataFrame({"System": [system], "Cluster": [cluster], "Drug": [drug], 
                                     "Res-P": [pval_res], "Sen-P": [pval_sen], "Acc-P": [pval_acc]})
                cluster_drug_sig.append(data)

        cluster_drug_sig = pd.concat(cluster_drug_sig, ignore_index = True)

        return(cluster_drug_sig)
